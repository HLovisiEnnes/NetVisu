...
def pca_network(number_of_layers, neurons_per_hidden_layer, activation, data, target,
                inputs_shape = 3, number_of_epochs = 100, test_size = 0.15):
    '''
    Plots the pca output of each hidden layer to 3 dimensions (if the dimensions are bigger than 3) for a neural network model
    Input: number_of_layers: number of hidden layers in the network; neurons_per_hidden_layer: width of each hidden layer, which must be the
    same for all layers; activation: activation function for each hidden layer, which must be the same; data: dataset; target: dataset's 
    labels; inputs_shape = 3: shape of input set to 3 by default number_of_epochs=100: number of training epochs, set by default to 100;
    test_size = 0.15: selects percentage of dataset to be used as validation data, set to 0.15
    '''
    if neurons_per_hidden_layer>3:
        model = feedforward(number_of_layers, neurons_per_hidden_layer, activation, inputs_shape = inputs_shape)
        trains_network(model, number_of_epochs, data, target, verbose = 0)
        pca = PCA(n_components=3)

        for i in range(number_of_layers):
            layer = tf.keras.models.Model(inputs=model.input,outputs=model.layers[i].output)
        ...
        raise(Exception("Layers width too narrow!"))

#define function that plots persistence homology study of each layer output
def persis_homol_network(number_of_layers, neurons_per_hidden_layer, activation, data, target, intrinsic_dimension = 2, pers_dis = 1,
                inputs_shape = 3, number_of_epochs = 100, test_size = 0.15):
    '''
    Plots the persitence homology barcodes and diagrams of each layer of the neural network
    Input:  number_of_layers: number of hidden layers in the network; neurons_per_hidden_layer: width of each hidden layer, which must be
    the same for all layers; activation: activation function for each hidden layer, which must be the same; data: dataset; target: dataset's 
    ...
    inputs_shape = 3: shape of input set to 3 by default number_of_epochs=100: number of training epochs, set by default to 100;
    test_size = 0.15: selects percentage of dataset to be used as validation data, set to 0.15 
    '''
    model = feedforward(number_of_layers, neurons_per_hidden_layer, activation, inputs_shape = inputs_shape)
    trains_network(model, number_of_epochs, data, target, verbose = 0)
...
def pca_and_persistence(number_of_layers, neurons_per_hidden_layer, activation, data, target, intrinsic_dimension = 2, pers_dis = 1,
                inputs_shape = 3, number_of_epochs = 100, test_size = 0.15):
    '''
    Plots both the pca and the persitence homology barcodes and diagrams of each layer of the neural network, good for avoiding the
    stochastic effect of the neural network model
    Input:  number_of_layers: number of hidden layers in the network; neurons_per_hidden_layer: width of each hidden layer, which must be
    the same for all layers; activation: activation function for each hidden layer, which must be the same; data: dataset; target: dataset's 
    labels; intrinsic_dimension = 1: intrinsic manifold dimension, so we know up to where perform the persistence homology;
    per_dis = 1: maximum distance to calculate persistence homology,
    ...
    if neurons_per_hidden_layer>3:
        ...
        for i in range(number_of_layers):
            ...
            scatter = ax.scatter(red[:,0], red[:,1], red[:,2],c='r')
            scatter = ax.scatter(blue[:,0], blue[:,1], blue[:,2],c='b')
            rips = gudhi.RipsComplex(points = layer_output, max_edge_length = 1)
